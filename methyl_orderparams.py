...
parser.add_argument("-t", "--topology", dest = "topology", default = "../redSqrR.prmtop", help="Topology for MD trajectory")
parser.add_argument("-x", "--trajectory", dest = "trajectory", default = ['../redSqrR.MD3.nc','../redSqrR.MD7.nc','../redSqrR.MD8.nc','../redSqrR.MD9.nc','../redSqrR.MD10.nc'], help="Python list of trayectory names")
...
#Cuales son los metilos marcados presentes en una proteina?
...
gen_list=[]
#ahora que tenemos los atomos de ILVAM involucrados en metilos, buscamos su XYZ
...
vector_list=[]
print("Methyl groups identified")
order_params_methyl=[]
        ...
        if atom.resName==option:
            for bond in pair:
               if atom.atName==bond[0]:
                   ...
                   S2=1.5*(np.mean(X**2)**2+np.mean(Y**2)**2+np.mean(Z**2)**2+2*np.mean(X*Z)**2+2*np.mean(X*Y)**2+2*np.mean(Y*Z)**2)-0.5
                   normalized_vector=[appendable[0],appendable[1],appendable[2],appendable[8],S2] 
                   order_params_methyl.append(normalized_vector)
print("Order parameters calculated")
